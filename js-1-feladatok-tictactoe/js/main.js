...
let rows = 3;
let mark = "X";
const allCells = document.querySelectorAll(".tictactoe .cell");
...
*/
const deleteSigns = () => {
  allCells.forEach((i) => (i.textContent = ""));
};
...
const addClickListener = () => {
  allCells.forEach((i) => i.addEventListener("click", handleClick));
};
...
const removeAllClickListeners = () => {
  allCells.forEach((i) => i.removeEventListener("click", handleClick));
};
...
const checkValues = (array) => {
  return (
    array
      .map((row) => {
        /*Ide írd a kódot!*/
        return row.every((i) => i == "X") || row.every((i) => i == "O");
      })
      .indexOf(true) !== -1
  );
};
...
const checkAllCellPlayed = (array) => {
  return array.flat().indexOf(null) == -1 ? true : false;
};
...
const checkWinner = () => {
  ...
  console.log(checkValues(matrix), checkColumnValues(), checkDiagonalValues());
  /*
  ...
  */
  if (checkValues(matrix) || checkColumnValues() || checkDiagonalValues()) {
    endGame(`The winner is Player ${mark === "X" ? "O" : "X"}.`);
  }
  if (checkAllCellPlayed(matrix)) {
    endGame("Draw! No winner");
  }
};
